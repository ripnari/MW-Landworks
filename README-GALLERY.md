...
### Step 4 — Upload the Snow Removal service image (one time)

The Snow Removal card on the homepage references a specific image at a known URL. To make it appear, upload that image once to Cloudinary with a specific name:

1. In Cloudinary, click **Upload** in the Media Library
2. Drag in the snow plowing photo you want to use for the Snow Removal service card
3. **Before clicking the final Upload button**, click **Advanced** at the bottom of the upload dialog
4. Set **Public ID** to exactly: `service-snow`
5. Click Upload

That's it. The website will now load the snow image from `https://res.cloudinary.com/mwlandworks/image/upload/service-snow`. If you ever want to swap the snow image, just upload a new one and overwrite the same public_id.

**Don't tag this image with `mwlandworks-gallery`** unless you also want it appearing in the project gallery alongside finished work — it's a service marketing photo, not a portfolio piece.
